...
class Topic2Controller extends Controller
{
    ...
    public function videoDetail(Request $request, Topic2Chapter $chapter)
    {
        $chapter->load(['topic2' => function ($query) {
            $query->with('bookCategory');
        }]);

        $videos = \App\Models\Topic2Video::where('topics2_chapters_id', $chapter->id)
            ->orderBy('seq')
            ->get();
        ...
        return Inertia::render('Topic2/VideoDetail', [
            'chapter' => $chapter,
            'items' => $items,
    ...
    public function storeVideo(Request $request, Topic2Chapter $chapter)
    {
        $request->validate([
        ...
        if (! $position || $position > $total) {
            ...
            \App\Models\Topic2Video::where('topics2_chapters_id', $chapter->id)
                ->where('seq', '>=', $position)
                ->increment('seq');
    ...
    {
        $topic2Video = \App\Models\Topic2Video::findOrFail($id);
        ...
        if (isset($request->seq)) {
            ...
            DB::transaction(function () use ($topic2Video, $targetPosition) {
                $allItems = \App\Models\Topic2Video::where('topics2_chapters_id', $topic2Video->topics2_chapters_id)
                    ->orderBy('seq')
                    ->lockForUpdate()
                    ->get();

                $currentIndex = $allItems->search(fn ($v) => $v->id === $topic2Video->id);
                if ($currentIndex === false) {
                    return;
                }
                ...
                if ($targetPosition > $currentPosition) {
                    ...
                    \App\Models\Topic2Video::where('topics2_chapters_id', $topic2Video->topics2_chapters_id)
                        ->whereBetween('seq', [$movingItem->seq + 1, $targetSeq])
                        ->decrement('seq');
