...
const handleSubmit = () => {
    if (modalType.value === 'add') {
        ...
        form.post(`/topic2/chapter/${props.chapter.id}/video`, {
            onSuccess: () => {
        ...
        form.put(`/topic2/video/${selectedItem.value.id}`, {
            onSuccess: () => {
                modalOpen.value = false;
                form.reset();
            },
...
const handleDelete = () => {
    if (selectedItem.value) {
        router.delete(`/topic2/video/${selectedItem.value.id}`, {
...
<template>
    <Head title="Topic 2 - {{ chapter.title }}" />
            ...
            <div class="flex h-full flex-col overflow-hidden rounded bg-white shadow">
                ...
                <div class="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-4 py-3">
                    <div class="flex items-center gap-2">
                        <span class="text-sm text-gray-600">{{ chapter.topic2?.bookCategory?.title || 'N/A' }} / {{ chapter.topic2?.title || 'N/A' }} / {{ chapter.title }}</span>
                    </div>
                    <Button 
                        @click="openModal('add')"
