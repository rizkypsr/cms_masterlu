...
const navigateToContent = (chapterId: number) => {
    window.open(`/topic3/chapter/${chapterId}/content`, '_blank');
};

// Video modal states
const videoModalOpen = ref(false);
const selectedChapterForVideo = ref<Topic3Chapter | null>(null);
...
const openVideoModal = (chapter: Topic3Chapter) => {
...
const handleVideoSubmit = () => {
    ...
    router.put(`/topic3/chapter/${selectedChapterForVideo.value.id}/video`, {
...
onMounted(() => {
    if (leftPanelScroll.value) {
        const savedScroll = sessionStorage.getItem('topic3-category-left-panel');
        if (savedScroll) {
...
const saveLeftPanelScroll = () => {
    if (leftPanelScroll.value) {
        sessionStorage.setItem('topic3-category-left-panel', leftPanelScroll.value.scrollTop.toString());
    }
};
</script>
                            ...
                            >
                                ...
                                <div v-if="category.topics3?.length">
                                    <div
                                        ...
                                        @click="router.visit(`/topic3/${topic3.id}`, { preserveScroll: true })"
                                    >
