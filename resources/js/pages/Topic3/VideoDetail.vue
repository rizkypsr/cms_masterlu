...
const selectedItem = ref<VideoItem | null>(null);
const selectedAvailableItem = ref<AvailableItem | null>(null);

// Form
const form = useForm({
    video_category_id: '' as string | number,
...
const openModal = (type: typeof modalType.value, item?: VideoItem) => {
...
const selectAvailableItem = (item: AvailableItem) => {
    ...
    form.video_category_id = item.id;
};
...
const handleSubmit = () => {
    if (modalType.value === 'add') {
        ...
        form.post(`/topic3/chapter/${props.chapter.id}/video`, {
            onSuccess: () => {
        ...
        form.put(`/topic3/video/${selectedItem.value.id}`, {
            onSuccess: () => {
                modalOpen.value = false;
                form.reset();
            },
...
const handleDelete = () => {
    if (selectedItem.value) {
        router.delete(`/topic3/video/${selectedItem.value.id}`, {
            onSuccess: () => modalOpen.value = false,
        });
    }
};

const navigateToVideoCategory = (videoCategoryId: number) => {
    window.open(`/video/video-category/${videoCategoryId}`, '_blank');
};
</script>

<template>
    <Head title="Topic 3 - {{ chapter.title }}" />
            ...
            <div class="flex h-full flex-col overflow-hidden rounded bg-white shadow">
                ...
                <div class="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-4 py-3">
                    <div class="flex items-center gap-2">
                        <span class="text-sm text-gray-600">{{ chapter.topic3?.bookCategory?.title || 'N/A' }} / {{ chapter.topic3?.title || 'N/A' }} / {{ chapter.title }}</span>
                    </div>
                    <Button 
                        @click="openModal('add')"
                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            <tr 
                                ...
                                :class="item.video_category_id ? 'cursor-pointer' : ''"
                                @click="item.video_category_id ? navigateToVideoCategory(item.video_category_id) : null"
                            >
